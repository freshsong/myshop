...
const prcode = $("#prcode").val();
   ...
   $('.size').change(function(){
    ...
    //console.log(quantityArray); //배포후에는 이렇게 수식이 제대로됐나 확인(배열이 제대로 들어와있나 확인)
   ...
   //$('#qup').click(function(){
    $(document).on('click', '.qup', function(){  //예전엔 live로 썼는데 on으로 통일시킴
      ...
      let ttmoney = tmoney * quantity; //totalmoney를 일일히 계산해야함  //(상품가격+옵션)*수량
      ttmoney = ttmoney.toLocaleString(); //세자리 콤마
      $(this).parents('.add-opt').find('.tomoney').html(ttmoney + "원"); //출력
      
      //let ind = $('.qup').index(this); (컬러선택 수량up 하면 가격변동 세트1)
      //$('.tomoney').eq(ind).html(tmoney + "원"); (컬러선택 수량up 하면 가격변동 세트2)
